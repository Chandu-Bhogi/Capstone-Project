...
const NavMenuLayout = ({ children, showMenu = true }) => {
  ...
  const [FundsModal, toggleFundsModal] = useState(false);
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item key="1">
        <div
          onClick={() => {
            toggleFundsModal((prevState) => !prevState);
          }}
        >
          Add Funds
        </div>
      ...
      <Menu.Item key="2">
        <div
          onClick={() => {
            toggleProfileModal((prevState) => !prevState);
          }}
        >
          Profile
        </div>
  ...
  return (
    <React.Fragment>
      ...
      <ChangePassword
        ...
        handleCancel={() => togglePasswordModal(false)}
      ></ChangePassword>
      <AddFunds
        visible={FundsModal}
        handleCancel={() => toggleFundsModal(false)}
      />
      <EditProfile
        visible={ProfileModal}
        handleCancel={() => toggleProfileModal(false)}
      ></EditProfile>
    </React.Fragment>
  );
};
